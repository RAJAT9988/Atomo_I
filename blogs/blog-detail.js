...
// // });
// document.addEventListener('DOMContentLoaded', async () => {
//     // Hamburger Menu Functionality (keep your existing code)
//     function initializeHamburgerMenu() {
//         const hamburger = document.getElementById('hamburger');
//         const navList = document.querySelector('#navbar ul');
...
//         const urlParams = new URLSearchParams(window.location.search);
//         const blogSlug = urlParams.get('slug') || urlParams.get('id'); // Support both
//         if (!blogSlug) throw new Error('No blog slug or ID provided in URL');
//         debugLog(`Loading blog with slug/ID: ${blogSlug}`);
...
//         // FIXED: Check slug first, then id
//         const blog = allBlogs.find(b => b.slug === blogSlug || b.id === blogSlug);
//         if (!blog) throw new Error(`Blog not found with slug/ID: ${blogSlug}`);
//         debugLog(`Found blog: ${blog.title} (slug: ${blog.slug}, id: ${blog.id})`);
...
//         const imageFile = blog.image || 'default-author.jpg';
//         const imagePath = getImagePath(imageFile);
//         debugLog(`Computed image path: ${imagePath}`);
